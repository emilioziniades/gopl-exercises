...
func Test(t *testing.T) {
	...
	visits := make(map[person]int)
	visits[person{"hufi", 26}] = 23
	visits[person{"mom", 45}] = 12
	Display("map[person]int", visits)

	baskets := make(map[[2]string]int)
	baskets[[2]string{"apples", "oranges"}] = 12
	baskets[[2]string{"lambo", "airpods"}] = 420000
	Display("baskets", baskets)
}
